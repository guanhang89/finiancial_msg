...
# 在开发机（已装这些包）执行：
$QMT_SITE = "C:\QMT交易端\bin.x64\Lib\site-packages"
...
把以下内容复制到大 QMT 的 `python` 目录（如 `C:\QMT交易端\python\`）：
...
def _known_qmt_python_dir():
    root = "".join(chr(value) for value in (0x56fd, 0x91d1, 0x8bc1, 0x5238))   # 某券商
    suffix = "".join(chr(value) for value in (0x4ea4, 0x6613, 0x7aef))          # 交易端
    return "D:\\" + root + "QMT" + suffix + "\\python"
    # 解码后 = C:\QMT交易端\python
```

- **`chr()` 编码**是为了规避 QMT 用 GBK 保存策略文件时中文乱码（用 Unicode 码点拼出"某券商交易端"）。
...
```
[bigqmt_shell] reload entry paths=['D:\\QMT交易端\\python']
...
r = redis.Redis(host="127.0.0.1", port=6379, db=5, password="...")
...
官方 `xtquant.xttrader.XtQuantTrader` 依赖客户端侧 XtQuantServer 通道。当前大 QMT 环境中直接连 `connect()` 返回 `-1`；大 QMT 的 `58600` 端口是 FormulaServer 不是行情服务。
