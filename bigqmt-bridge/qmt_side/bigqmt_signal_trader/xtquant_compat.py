...
class BigQmtXtTrader:
    def __init__(
        ...
        # 执行事件注册表：user_order_id(bq:...) -> {stock_code, status, filled_volume,
        # filled_amount, order_sys_id}；_sysid_index 为 sysid -> user_order_id 反查。
        # bigqmt 的 order_stock 先返回用户ID、sysid 随 order 事件异步到达，
        # 轮询确认/撤单都以此注册表为准（query 路径在某些环境不可用）。
        self._order_exec = {}
        self._sysid_index = {}
    ...
    def _learn_exec_event(self, event):
        """从 order/trade 事件学习订单状态与成交累计，供轮询确认和撤单映射使用。"""
        etype = event.get("event_type")
        uid = str(event.get("user_order_id") or event.get("remark") or "")
        sysid = str(event.get("order_sys_id") or event.get("order_sysid") or "")
        # 已知 sysid 归属的事件（如 trade 只带 sysid）直接归入 uid 键
        key = uid or self._sysid_index.get(sysid, "") or sysid
        if not key:
            return
        # 同一订单曾以 sysid 为键（如 trade 事件先到），合并到 uid 键下
        if uid and sysid and sysid != uid:
            orphan = self._order_exec.pop(sysid, None)
            if orphan:
                base = self._order_exec.setdefault(key, {})
                base["filled_volume"] = max(int(base.get("filled_volume") or 0),
                                            int(orphan.get("filled_volume") or 0))
                base["filled_amount"] = max(float(base.get("filled_amount") or 0.0),
                                            float(orphan.get("filled_amount") or 0.0))
                if base.get("status") is None:
                    base["status"] = orphan.get("status")
                if not base.get("stock_code"):
                    base["stock_code"] = orphan.get("stock_code", "")
        rec = self._order_exec.setdefault(key, {"stock_code": "", "status": None,
                                                "filled_volume": 0, "filled_amount": 0.0,
                                                "order_sys_id": ""})
        if event.get("stock_code"):
            rec["stock_code"] = str(event.get("stock_code"))
        if sysid:
            rec["order_sys_id"] = sysid
            self._sysid_index[sysid] = key
        if etype == "order":
            status = _safe_int(event.get("status"), None)
            if status is not None:
                rec["status"] = status
            traded = _safe_int(event.get("traded_volume"), None)
            if traded is not None:
                rec["filled_volume"] = max(int(rec.get("filled_volume") or 0), traded)
        elif etype == "trade":
            vol = _safe_int(event.get("volume"), 0) or 0
            px = _safe_float(event.get("price"), 0.0) or 0.0
            rec["filled_volume"] = int(rec.get("filled_volume") or 0) + vol
            rec["filled_amount"] = float(rec.get("filled_amount") or 0.0) + vol * px

    def order_exec_snapshot(self, order_id):
        """按 user_order_id 或 order_sysid 查执行注册表快照；无记录返回 None。"""
        oid = str(order_id or "")
        if not oid:
            return None
        rec = self._order_exec.get(oid)
        if rec is None and oid in self._sysid_index:
            rec = self._order_exec.get(self._sysid_index[oid])
        if rec is None:
            return None
        filled = int(rec.get("filled_volume") or 0)
        avg = (float(rec.get("filled_amount") or 0.0) / filled) if filled else 0.0
        return {"order_id": oid,
                "order_sys_id": rec.get("order_sys_id") or "",
                "stock_code": rec.get("stock_code") or "",
                "status": rec.get("status"),
                "filled_volume": filled,
                "avg_price": avg}

    def inflight_orders(self, stock_code=None):
        """注册表中仍处于在途状态(48未报/49待报/50已报/55部成)的委托。"""
        out = []
        for oid, rec in self._order_exec.items():
            if rec.get("status") in (48, 49, 50, 55):
                if stock_code is None or rec.get("stock_code") == str(stock_code):
                    item = {"order_id": oid}
                    item.update(rec)
                    out.append(item)
        return out
    ...
    def cancel_order_stock(self, account, order_id):
        oid = str(order_id or "")
        # 1) 事件注册表学到的 user_order_id -> order_sysid 映射（最可靠）
        rec = self._order_exec.get(oid)
        if rec is None and oid in self._sysid_index:
            rec = self._order_exec.get(self._sysid_index[oid])
        if rec and rec.get("order_sys_id"):
            return self.cancel_order_stock_sysid(account, "", rec["order_sys_id"])
        # 2) 查询兜底：order_id/order_sysid/order_remark 三重匹配
        try:
            for o in self.query_stock_orders(account, cancelable_only=True):
                if oid in (str(getattr(o, "order_id", "") or ""),
                           str(getattr(o, "order_sysid", "") or ""),
                           str(getattr(o, "order_remark", "") or "")):
                    sysid = str(getattr(o, "order_sysid", "") or "")
                    if sysid:
                        return self.cancel_order_stock_sysid(account, "", sysid)
        except Exception:
            pass
        # 3) 原样透传（兼容直接传 sysid 的场景）
